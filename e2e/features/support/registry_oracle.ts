...
const registryOracle = {
    ...
    async getLatestVersion(registry: string, image: string, pattern: string = '.*'): Promise<string> {
        ...
        try {
            ...
            } else if (registry === 'ghcr.public' || registry === 'ghcr.private' || registry === 'gitlab.private' || registry === 'lscr.private') {
                let url: string | null | undefined;
                let headers: any = {};

                if (registry === 'ghcr.public' || registry === 'ghcr.private' || registry === 'lscr.private') {
    ...
    async getLatestDigest(registry: string, image: string, tag: string = 'latest'): Promise<string> {
        try {
            ...
            if (registry === 'hub.public') {
                ...
                // If it's a manifest list / index, find the manifest matching the local image's architecture
                if (response.manifests) {
                    const arch = getLocalImageArchitecture(`${repo}:${tag}`) ?? getDockerArchitecture();
                    const manifest = response.manifests.find((m: any) =>
                        m.platform && m.platform.architecture === arch && m.platform.os === 'linux'
                    );
                    if (manifest) {
