...
- :warning: **Containers whose compose pin only *substring*-matched are no longer members of that compose file.** A file pinning `podinfo:5.0.00` no longer matches a container on `5.0.0`, and a combined `repo:tag@sha256:...` pin no longer matches a tag-form container — matching is now exact after canonicalization. Such a container is no longer batchable through the `dockercompose` trigger, so an explicit batch trigger request naming it returns **HTTP 400** instead of silently rewriting the wrong service's line. Audit compose pins that differ from the running tag before upgrading
