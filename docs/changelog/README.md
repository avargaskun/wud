...
## next
- :fire: [REGISTRY] - Retry throttled and transient registry responses (fixes #59). A registry request answered with HTTP `429`, `502`, `503` or `504` is now retried up to 3 times with exponential backoff and jitter, instead of failing the container for the whole watch cycle. The server's hint is honoured as a minimum wait — the `Retry-After` header, or the `retry-after: <duration>` GHCR embeds in its error message — with a 250 ms floor; a hint above 30 s is not waited for and fails as before. Other statuses and network errors are not retried
  - A retried request that recovers only logs at `debug`. The `Request failed with status code [...]` warnings now appear once, for the final attempt, when retries are exhausted
  - New counter `wud_registry_retry_count{type,name,status}`; `wud_registry_response_count` keeps counting every attempt. The watcher's summary line gains a field: `Cron finished (70 containers watched, 0 errors, 3 available updates, 9 registry retries)`. The figure is process-wide, so watchers scanning at the same time on one instance may each include the other's retries
