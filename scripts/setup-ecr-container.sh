...
#             * Uses 'nginx:alpine' so containers stay running when started.
#          3. Creates a restricted IAM user.
#          4. DELETES any existing keys for that user (clean slate).
#          5. Generates and saves new access keys.
#
# USAGE:   ./setup_ecr_test.sh
#
# REQ:     AWS CLI configured with Admin permissions.
...
# --- STEP 4: PREPARE AND PUSH IMAGES ---
echo -e "\n${GREEN}Step 3: pushing test images (v1.0.0 and v2.0.0)...${NC}"
echo "Using 'nginx:alpine' so containers stay running during tests."
...
# 7a. List existing keys (using query to get raw text IDs)
...
# We use grep and awk to extract values from the JSON file to avoid requiring 'jq'
...
# Cleanup the credentials file for security (Optional - comment out if you want to keep it)
rm "$CRED_FILE"